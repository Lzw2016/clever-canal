...
import org.clever.canal.parse.dbsync.binlog.CharsetConversion;
...
public final class UserVarLogEvent extends LogEvent {
     ...
     *
     * @since mysql-5.1.60//include/mysql_com.h
     */
    ...
    public UserVarLogEvent(LogHeader header, LogBuffer buffer, FormatDescriptionLogEvent descriptionEvent)
        ...
        if (isNull) {
            ...
            switch (type) {
                case REAL_RESULT:
                    value = buffer.getDouble64(); // float8get
                    break;
                case INT_RESULT:
                    if (valueLen == 8) value = buffer.getLong64(); // !uint8korr
                    else if (valueLen == 4) value = buffer.getUint32();
