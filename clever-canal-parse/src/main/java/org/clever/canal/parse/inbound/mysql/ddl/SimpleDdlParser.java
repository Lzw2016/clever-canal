...
import org.clever.canal.filter.PatternUtils;
...
public class SimpleDdlParser {
    ...
    private static boolean isDml(String queryString, String pattern) {
        Perl5Matcher matcher = new Perl5Matcher();
        return matcher.matches(queryString, PatternUtils.getPattern(pattern));
    }
    ...
    private static String removeComment(String sql) {
        ...
        while (true) {
            ...
            sql = sql.substring(0, index0) + " " + sql.substring(index1 + end.length());
        }
    }
}
