...
public class LoadLogEvent extends LogEvent {
    ...
    public LoadLogEvent(LogHeader header, LogBuffer buffer, FormatDescriptionLogEvent descriptionEvent) {
        ...
        copyLogEvent(
                buffer,
                ((header.type == LOAD_EVENT) ? loadHeaderLen + descriptionEvent.commonHeaderLen : loadHeaderLen + FormatDescriptionLogEvent.LOG_EVENT_HEADER_LEN),
                descriptionEvent
        );
    }

    /**
     * @since mysql-5.1.60/sql/log_event.cc - Load_log_event::copy_log_event
     */
