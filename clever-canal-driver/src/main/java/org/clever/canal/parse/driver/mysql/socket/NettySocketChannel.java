...
public class NettySocketChannel implements SocketChannel {
            ...
            if (readSize > cache.readableBytes()) {
                ...
                if (timeout > 0) {
                    ...
                    if (accumulatedWaitTime > timeout) {
                        String sb = "socket read timeout occured !" + " readSize = " + readSize + ", readableBytes = " + cache.readableBytes() + ", timeout = " + timeout;
    ...
    @Override
    public void read(byte[] data, int off, int len, int timeout) {
